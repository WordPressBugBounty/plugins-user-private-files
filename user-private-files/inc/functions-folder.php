...
if (!function_exists('upvf_check_fldr_access')) {
	function upvf_check_fldr_access($fldr_id) {

		$curr_user_id = get_current_user_id();
		$curr_allowed_users = get_post_meta($fldr_id, 'upf_allowed', true);
		$doc_author = get_post_field('post_author', $fldr_id);
	   
		return $curr_user_id == $doc_author || (is_array($curr_allowed_users) && in_array($curr_user_id, $curr_allowed_users));
	}
}

if (!function_exists('upvf_check_fldr_full_access')) {
	function upvf_check_fldr_full_access($fldr_id) {

		$curr_user_id = get_current_user_id();
		$curr_file_acs_users = get_post_meta($fldr_id, 'upf_acs_full', true);
		$doc_author = get_post_field('post_author', $fldr_id);
	...
	function upvf_pro_load_flder_callback(){
		...
		if(isset($_POST['fldr_id'])){
            ...
            if($fldr_id == 'all-shared-files' || $fldr_id == 'trash-files' || $fldr_id == 'all-files' || $fldr_id == 'filter-shared') {
                ...
                $allwd = upvf_check_fldr_access($fldr_id);
            }
...
if (!function_exists('upvf_pro_rename_folder')) {
	function upvf_pro_rename_folder(){
		...
		if(upvf_check_fldr_full_access($folder_id)){ // checking permission
...
if (!function_exists('upvf_pro_share_folder')) {
	function upvf_pro_share_folder() {
		...
		if(upvf_check_fldr_full_access($fldr_id)){ // checking permission
