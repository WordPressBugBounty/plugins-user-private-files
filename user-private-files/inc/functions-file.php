...
if (!function_exists('upvf_check_file_access')) {
	function upvf_check_file_access($doc_id) {

		$curr_user_id = get_current_user_id();
		$curr_allowed_users = get_post_meta($doc_id, 'upf_allowed', true);
		$doc_author = get_post_field('post_author', $doc_id);
	   
		return $curr_user_id == $doc_author || (is_array($curr_allowed_users) && in_array($curr_user_id, $curr_allowed_users));
	}
}

if (!function_exists('upvf_check_file_full_access')) {
	function upvf_check_file_full_access($doc_id) {

		$curr_user_id = get_current_user_id();
		$curr_file_acs_users = get_post_meta($doc_id, 'upf_acs_full', true);
		$doc_author = get_post_field('post_author', $doc_id);
	
		return $curr_user_id == $doc_author || (is_array($curr_file_acs_users) && in_array($curr_user_id, $curr_file_acs_users));
	}
}
...
if (!function_exists('upvf_pro_preview_file')) {
	function upvf_pro_preview_file() {
		...
		if(isset($_POST['doc_id'])){
            ...
            if (upvf_check_file_access($doc_id)){
                ...
                if(!$inside_shared){
                    ...
                    if($curr_allowed_users){
                        foreach($curr_allowed_users as $alwd_usr){
                            ...
                            if($alwd_usr_obj){
                                ...
                                if (is_array($curr_file_acs_users) && in_array($alwd_usr, $curr_file_acs_users)){
                    ...
                    if($curr_file_acs_users){
                        ...
                        if (is_array($curr_file_acs_users) && in_array($req_user_id, $curr_file_acs_users)){
...
if (!function_exists('upvf_pro_update_doc')) {
	function upvf_pro_update_doc() {
		...
		if(upvf_check_file_full_access($docid)){ // checking permission
...
if (!function_exists('upvf_pro_rename_file')) {
	function upvf_pro_rename_file(){
		...
		if(upvf_check_file_full_access($file_id)){ // checking permission
			if($file_id != 0){
				$file_args = array(
					'ID'           => $file_id,
...
if (!function_exists('upvf_pro_move_file')) {
	function upvf_pro_move_file(){
		...
		$curr_user_id = get_current_user_id();
		$doc_author = get_post_field ('post_author', $doc_id);
		$target_allowed = ($fldr_id == 'all-files') || upvf_check_fldr_full_access($fldr_id);
		if($curr_user_id == $doc_author && $target_allowed){ // checking permission on both file and destination folder
...
if (!function_exists('upvf_pro_file_add_cmnt')) {
	function upvf_pro_file_add_cmnt(){
		...
		if(upvf_check_file_access($file_id)){
			if($file_id != 0){
				$cmnt_args = array(
